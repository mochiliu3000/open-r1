    ...
    with Pipeline().ray() as pipeline:
        TextGeneration(
            llm = vLLM(
                model = model,
                tokenizer = model,
                cuda_devices = [int(device) for device in cuda_devices.split(",")],
                extra_kwargs = {
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        help="CUDA devices to use for generation",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        required=False,
        help="Maximum model length for generation",
    )
    ...
    if args.hf_save_dir:
        print(f"Saving resulting dataset to '{args.hf_save_dir}'...")
        distiset.save_to_disk(args.hf_save_dir)
        print("Dataset saved!")
    ...
    print("-" * 40)
    print(distiset['default']['train']['generation'][0])


    ''' Example Command:
    python mochi_r1/generate_vllm.py \
        --hf-dataset "/home/jovyan/liumochi/open-r1/data/AI-MO/NuminaMath-TIR/data" \
        ...
        --model "/home/jovyan/liumochi/open-r1/model/deepseek-ai/DeepSeek-R1-Distill-Qwen-7B" \
        --temperature 0.6 \
        --top-p 0.9 \
        --max-new-tokens 8192 \
        --max-model-len 8192 \
